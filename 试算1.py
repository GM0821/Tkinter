...
filemenu.add_command(label='打开'),filemenu.add_command(label='新建'),filemenu.add_command(label='保存')
...
left_frame,right_frame,bottom_frame = ttk.Frame(pw_1,width=700,relief='flat'),\
                                      ttk.Frame(pw_1,height=600,relief='flat'),\
...
scale.pack(pady=2),labeledscale.pack(pady=2),spinbox.pack(pady=2),\
labelframe.pack(pady=2),scrollbar.pack(side='right',pady=2,fill='y'),listbox.pack(side='left',pady=2),\
...
menubutton.pack()
separator_right = ttk.Separator(right_frame).pack(fill='x',padx=5)
mb_menu = tk.Menu(menubutton,tearoff=0)
mb_menu.add_command(label='命令按钮'),mb_menu.add_radiobutton(label='单选按钮'),\
...
notebook.pack(side='left',padx=5),canvas.pack(side='left',padx=5)
...
balloon.bind_widget(menubutton,balloonmsg='这是一个汽泡提示')
